...
def upgrade() -> None:
    ...
    # classrooms 依赖 schools + users；users.class_id 又依赖 classrooms（循环依赖）。
    # 先建 classrooms，再回补 users.class_id 外键，避免 MySQL 建表时报 1824。
    ...
    if not is_sqlite:
        # SQLite 不支持 ALTER TABLE ADD FOREIGN KEY（且默认不强制外键），跳过回补
        op.create_foreign_key('fk_users_class_id', 'users', 'classrooms', ['class_id'], ['id'])
...
def downgrade() -> None:
    ...
    # 先删回补的 users.class_id 外键，再删 classrooms（classrooms.teacher_id 引用 users）
    if not is_sqlite:
