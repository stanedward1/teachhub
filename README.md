...
- `frontend/nginx.conf`：用 `realip` 模块（`set_real_ip_from` + `real_ip_recursive on`）
  ...
  **若你的上游代理不在其中（例如宿主机以 `10.x` / `192.168.x` 连入），必须把该网段补进
  `set_real_ip_from`** —— 否则 realip 不生效，`X-Real-IP` 会被写成**上游代理自身地址**，
  登录 IP 就记成代理地址：不再全是 `127.0.0.1`，但**仍然是错的**（只是错成了另一个固定值）。
  这类"非回环内层代理"也可以在**后端**用 `TRUSTED_PROXY_CIDRS` 声明（见下一条）。
- 后端 `routers/auth.py::_client_ip`：不再"取 XFF 最右一跳"，而是
  「优先取可信的 `X-Real-IP`；被自有代理地址覆盖时，退回 XFF 并跳过自有跳、取最左一跳」，
  因此多层代理也能还原出真实客户端。
  这里的"自有跳"**只按回环 / 链路本地 / 未指定判定**（`127.0.0.0/8`、`::1` 等），
  **不把 RFC1918 内网一律当自有**——因为真实客户端本身常常就在内网（校园网 `192.168.x`），
  若一并丢弃会丢掉**不可伪造**的 `X-Real-IP`、转而信任**可被客户端伪造**的 XFF。
  - 若内层代理以**非回环**地址连入（docker 网关 `172.17.0.1`、VPC `10.0.0.5` 等），
    这类地址与真实内网客户端在协议层无法区分，需在 `backend/.env` 声明：
    ```ini
    TRUSTED_PROXY_CIDRS=172.16.0.0/12,10.0.0.0/8
    ```
    （逗号分隔可填多个；默认留空即可。多数情况下 `frontend/nginx.conf` 的 realip 已代为还原。）
    ...
    - ⚠️ **只声明代理自身连入的网段，不要"把内网全列上"**：真实客户端本身常常也在内网
      （校园网 `192.168.x`）。若把整个 `192.168.0.0/16` 列为可信，同一内网的客户端就会被
      当作代理跳丢弃，转而采信可被伪造的 `X-Forwarded-For` —— 反而更不安全。
